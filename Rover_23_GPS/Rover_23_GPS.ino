...
void handleRoot() {
  ...
  //replace with url to frontend - currently just connects to a react app
  server.sendHeader("Access-Control-Allow-Origin", "http://localhost:3000");
  server.sendHeader("Access-Control-Allow-Methods", "GET, POST, OPTIONS");
  server.sendHeader("Access-Control-Allow-Headers", "*");

  if(uiValidGPSData == 247){
    String data;
    data += "{";
    data += "  \"GPS\": \""+ String(strGPS) + "\",";
    data += "  \"IMU\": \"" + String(strIMU) + "\"";
    data += "}";
  server.send(200, "text/javascript", data);
    ...
    String data;
    data += "{";
    data += "  \"GPS\": Invalid GPS Data - Error Code \"" + String(uiValidGPSData) + "\",";
    data += "  \"IMU\": \"" + String(strIMU) + "\"";
    data += "}";
    server.send(200, "text/javascript", data);
  }
...
void handleNotFound() {
  String message = "Error - URI " + server.uri() +  "not found";
...
void loop() {
  ...
  if ((ulCurrentMillis - ulPreviousMillis) >= ciGPS_IMUTimer) {
    ...
    sprintf(strIMU, "1,%s,%s,%s,%s,%s,%s,%s", strXAccel, strYAccel, strZAccel, strXGyro, strYGyro, strZGyro, strTemperature);

    //used to be 127
    if (uiValidGPSData == 0x00F7) {
      strcpy(strGPS, "0");
      ...
      sprintf(strGPS, "1,%i,%s,%s,%i,%s,%s,%s,%i,%s", uiTime, strLat, strLon, uiDate, strMPS, strDegree, strAlt, uiSatNumber, strHDOP);
    } else {
      strcpy(strGPS, "0");
    }
  }
